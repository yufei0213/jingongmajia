...
function drawStrongSolidLine(tar, p1, p2, color) {
    ...
    tar.lineWidth = 2 * getPixelRatio(tar);
    tar.stroke();
    tar.closePath();
};
...
DriverGrid.prototype.draw = function (events, active) {
    ...
    if (this.modifyEvent) {
        var driverEvents = this.modifyEvent;
    }
    var coordinate_x;
    var start = {};
    var end = {};
    ...
    if (active) {
        //每分钟刷新一次
        setInterval($.proxy(this.timer, this), 60 * 1000);
    }
};
...
DriverGrid.prototype.drawHour = function () {
    //绘制右侧的时间。初始化为0h 00m
    // this.ctx.font = 9 * this.x + "px Verdana";
    this.ctx.font = 18 * this.x + "px Verdana";
    ...
    this.ctx.fillText(texts.offDutyTimes, this.gridW + this.spaceX + (6 * this.x), this.cellhx * 1 + this.spaceY + (6 * this.x));
    this.ctx.fillText(texts.sleeperBerthTimes, this.gridW + this.spaceX + (6 * this.x), this.cellhx * 3 + this.spaceY + (6 * this.x));
    this.ctx.fillText(texts.drivingTimes, this.gridW + this.spaceX + (6 * this.x), this.cellhx * 5 + this.spaceY + (6 * this.x));
    this.ctx.fillText(texts.onDutyTimes, this.gridW + this.spaceX + (6 * this.x), this.cellhx * 7 + this.spaceY + (6 * this.x));
};

DriverGrid.prototype.modify = function (newEvent) {
    ...
    for (var i = 0; i < this.allEvents.length; i++) {
        ...
        if (this.allEvents[i].startSecond < newEvent.endSecond && this.allEvents[i].endSecond > newEvent.endSecond) {
...
DriverGrid.prototype.initCanvas = function () {
    ...
    //绘图使用的变量
    //x的大小
    // this.spaceX = 35 * this.x;
    this.spaceX = 70 * this.x;
    //y的大小
    // this.spaceY = 15 * this.x;
    this.spaceY = 30 * this.x;
    // this.gridW = this.w - (this.spaceX * 2) - 50;
    this.gridW = this.w - (this.spaceX * 2) - ((100 / 3) * this.x);
    ...
    // this.ctx.lineWidth = 0.5 * this.x;
    this.ctx.lineWidth = 1 * this.x;
    this.ctx.stroke();
    this.ctx.closePath();
    ...
    this.ctx.closePath();
    //绘制左侧的状态
    // this.ctx.font = 8 * this.x + "px Verdana";
    this.ctx.font = 16 * this.x + "px Verdana";
    this.ctx.fillStyle = '#333';
    ...
    this.ctx.fillText("OFF", this.spaceX - (6 * this.x), this.Y_OF + (6 * this.x));
    this.ctx.fillText("SB", this.spaceX - (6 * this.x), this.Y_SB + (6 * this.x));
    this.ctx.fillText("D", this.spaceX - (6 * this.x), this.Y_D + (6 * this.x));
    this.ctx.fillText("ODND", this.spaceX - (6 * this.x), this.Y_ODND + (6 * this.x));

    //绘制下面的提示线
    // this.ctx.font = 7 * this.x + "px Verdana";
    this.ctx.font = 14 * this.x + "px Verdana";
    this.ctx.fillStyle = '#333';
    this.ctx.textAlign = "start";
    ...
    this.ctx.fillText("Yard Move", this.cellw * 7 + this.spaceX + (3 * this.x), this.gridH + this.spaceY + (22 * this.x));
    this.ctx.fillText("Personal Use", this.cellw * 16 + this.spaceX + (3 * this.x), this.gridH + this.spaceY + (22 * this.x));
    ...
    this.ctx.moveTo(this.cellw * 3 + this.spaceX, this.gridH + this.spaceY + (18 * this.x));
    this.ctx.lineTo(this.cellw * 6.5 + this.spaceX, this.gridH + this.spaceY + (18 * this.x));
    this.ctx.strokeStyle = "#FF6D36";
    // this.ctx.lineWidth = 3;
    this.ctx.lineWidth = 2 * this.x;
    this.ctx.stroke();
    ...
    this.ctx.moveTo(this.cellw * 12 + this.spaceX, this.gridH + this.spaceY + (18 * this.x));
    this.ctx.lineTo(this.cellw * 15.5 + this.spaceX, this.gridH + this.spaceY + (18 * this.x));
